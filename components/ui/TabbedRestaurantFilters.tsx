  ...
  const FilterTabButton: React.FC<{ 
    ...
    >
      ...
      {id === 'search' && filters.search?.trim() && (
        <span className="ml-2 px-2 py-1 bg-amber-500 text-white text-xs font-medium rounded-full" aria-label={`Filtro ativo: ${filters.search}`}>
          {filters.search.trim()}
        </span>
      )}
          ...
          <div className="flex items-center space-x-4">
            ...
            >
              <div className="flex items-center justify-center w-10 h-10 bg-amber-100/80 rounded-xl group-hover:bg-amber-100 transition-colors duration-150">
                <FilterIcon className="h-5 w-5 text-amber-600 group-hover:text-amber-700" />
              </div>
