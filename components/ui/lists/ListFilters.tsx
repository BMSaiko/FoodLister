...
export default function ListFilters({ 
  ...
  return (
    <div className="space-y-4">
        ...
        <button
          ...
          className={`px-4 py-2 border rounded-lg flex items-center gap-2 transition-colors ${
            showFilters || hasActiveFilters 
              ? 'border-[var(--primary)] bg-[var(--primary-50)] text-[var(--primary-dark)]' 
              : 'border-[var(--gray-300)] hover:bg-[var(--gray-50)]'
          }`}
        >
      ...
      {/* Filter Options */}
      {showFilters && (
          <div className="bg-[var(--card-bg)] rounded-lg shadow-[var(--card-shadow)] p-4 space-y-4">
          {/* Tags */}
          {tags.length > 0 && (
            <div>
              <label className="block text-sm font-medium text-[var(--gray-700)] mb-2">Tags</label>
          ...
          {/* Privacy */}
          <div>
            <label className="block text-sm font-medium text-[var(--gray-700)] mb-2">Privacidade</label>
            <div className="flex gap-2">
              <button
              ...
              <button
                ...
                className={`px-3 py-1 rounded-full text-sm transition-colors ${
                  filters.isPublic === true 
                    ? 'bg-[var(--green-500)] text-[var(--primary-foreground)]' 
                    : 'bg-[var(--gray-100)] text-[var(--gray-700)] hover:bg-[var(--gray-200)]'
                }`}
              >
                Públicas
              ...
              <button
                ...
                className={`px-3 py-1 rounded-full text-sm transition-colors ${
                  filters.isPublic === false 
                    ? 'bg-[var(--red-500)] text-[var(--primary-foreground)]' 
                    : 'bg-[var(--gray-100)] text-[var(--gray-700)] hover:bg-[var(--gray-200)]'
                }`}
              >
                Privadas
          ...
          {hasActiveFilters && (
            <div className="flex justify-end">
              <button
                onClick={clearAll}
                className="flex items-center gap-1 text-sm text-[var(--gray-600)] hover:text-[var(--gray-800)] transition-colors"
              >
                <X className="h-3 w-3" />
                Limpar filtros
              </button>
