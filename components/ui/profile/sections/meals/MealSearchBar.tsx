...
export default function MealSearchBar({ onSearch, onClear, activeFiltersCount }: MealSearchBarProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-[var(--card-bg)] border border-[var(--gray-200)] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[var(--amber-500)] focus:border-transparent placeholder-[var(--gray-400)]"
          />
          {filters.searchQuery && (
            <button
              onClick={() => handleInputChange('searchQuery', '')}
              className="absolute right-3 top-1/2 -translate-y-1/2 p-0.5 rounded-full hover:bg-[var(--background-secondary)]"
            >
              <X className="h-3.5 w-3.5 text-[var(--gray-500)]" />
            </button>
          )}
        </div>
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors ${
            showFilters || hasActiveFilters
              ...
              : 'bg-[var(--card-bg)] border border-[var(--gray-200)] text-[var(--gray-300)] hover:bg-[var(--background-secondary)]'
          }`}
        >
          <Filter className="h-4 w-4" />
          <span className="hidden sm:inline">Filtros</span>
          {activeFiltersCount > 0 && (
            <span className="flex items-center justify-center min-w-[20px] h-5 px-1.5 bg-[var(--card-bg)] text-[var(--amber-600)] rounded-full text-xs font-bold">
              {activeFiltersCount}
            </span>
          )}
        </button>
      ...
      {/* Extended Filters */}
      {showFilters && (
        <div className="bg-[var(--background-secondary)] rounded-lg p-4 space-y-3 border border-[var(--gray-100)]">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {/* Date From */}
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-[var(--gray-500)] flex items-center gap-1.5">
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-[var(--card-bg)] border border-[var(--gray-200)] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[var(--amber-500)] focus:border-transparent"
              />
            </div>
          ...
          {hasActiveFilters && (
            <div className="flex justify-end pt-2 border-t border-[var(--gray-200)]">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-[var(--gray-500)] hover:text-[var(--gray-200)] hover:bg-[var(--background-secondary)] rounded-lg transition-colors"
              >
