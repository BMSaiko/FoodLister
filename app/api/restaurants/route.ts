...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let query = client.from('restaurants').select(
      ...
      'visited, created_at, updated_at, creator_id, creator_name, latitude, ' +
      'longitude, images, display_image_index, opening_hours, ' +
    ...
    let restaurants: Restaurant[] = (restaurantsData || []).map((r: any) => ({
      ...
      latitude: r.latitude, longitude: r.longitude, opening_hours: r.opening_hours,
    }));
