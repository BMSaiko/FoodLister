...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = admin.from('restaurants').select('id, name, description, image_url, price_per_person, rating, location, creator_name, created_at, review_count', { count: 'exact' });
    if (search) {
      query = query.or(`name.ilike.%${search}%`);
    }
    let { data: restaurants, count, error } = await query.order('created_at', { ascending: false }).range(from, to);
    ...
    if (error && error.code === '42703') {
      ...
      let fallbackQuery = admin.from('restaurants').select('id, name, description, image_url, price_per_person, rating, location, creator_name, created_at', { count: 'exact' });
      if (search) {
    ...
    if (error) {
      console.error('Admin restaurants - query error:', error.message || error);
      return NextResponse.json({ error: error.message || 'Database error' }, { status: 500 });
    }

    return NextResponse.json({ data: restaurants, total: count, page, limit });
  } catch (error: any) {
    console.error('Admin restaurants - catch error:', error.message || error, error.stack);
