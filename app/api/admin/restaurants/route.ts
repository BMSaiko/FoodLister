...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: restaurants, count, error } = await admin.from('restaurants').select('id, name, description, rating, created_at, creator_name, review_count', { count: 'exact' }).order('created_at', { ascending: false }).range(from, to);
